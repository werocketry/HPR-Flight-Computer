...
if "Alt" in df.columns:
    df.rename(columns={"Alt": "baroAlt"}, inplace=True)
"""-------------------------------
    ...
    for j in range(lastVal):
        if pd.notnull(df.at[j, "baroAlt"]):
            ...
            if filterFull == True:
                ...
                baroVel = (smoothAlt - smoothAltArray[arrayPosn]) / (
                    (df.at[j, "timeStamp"] - timeArray[arrayPosn]) / 1000000
                )
                smoothAltArray[arrayPosn] = smoothAlt

            else:
                smoothAlt = altSum / (arrayPosn + 1)
                baroVel = (smoothAlt - smoothAltArray[0]) / (
                    (df.at[j, "timeStamp"] - timeArray[0]) / 1000000
                )
                smoothAltArray[arrayPosn] = smoothAlt
...
if "aX" in df.columns:
    df.rename(columns={"aX": "accelZ"}, inplace=True)
if "aY" in df.columns:
    df.rename(columns={"aY": "accelY"}, inplace=True)
if "aZ" in df.columns:
    df.rename(columns={"aZ": "accelX"}, inplace=True)
if "gX" in df.columns:
    df.rename(columns={"gX": "gyroZ"}, inplace=True)
if "gY" in df.columns:
    df.rename(columns={"gY": "gyroY"}, inplace=True)
if "gZ" in df.columns:
    df.rename(columns={"gZ": "gyroX"}, inplace=True)
    df["gyroX"] *= -1
if "vel" in df.columns:
    df.rename(columns={"vel": "intVel"}, inplace=True)
if "alt" in df.columns:
    df.rename(columns={"alt": "intAlt"}, inplace=True)
if "analog" in df.columns:
    df.rename(columns={"analog": "smoothHighGz"}, inplace=True)
if "gps_alt" in df.columns:
    df.rename(columns={"gps_alt": "gpsAlt"}, inplace=True)
if "pitch" in df.columns:
    df.rename(columns={"pitch": "pitchX"}, inplace=True)
if "yaw" in df.columns:
    df.rename(columns={"yaw": "yawY"}, inplace=True)
if "roll" in df.columns:
    df.rename(columns={"roll": "rollZ"}, inplace=True)
if "rotnX" in df.columns:
    df.rename(columns={"rotnX": "rollZ"}, inplace=True)
if "rotnY" in df.columns:
    df.rename(columns={"rotnY": "yawY"}, inplace=True)
if "rotnZ" in df.columns:
    df.rename(columns={"rotnZ": "pitchX"}, inplace=True)
if "gpsAlt" in df.columns:
    df.rename(columns={"gpsAlt": "gnssAlt"}, inplace=True)
if "lat" in df.columns:
    df.rename(columns={"lat": "gpsLat"}, inplace=True)
if "lon" in df.columns:
    df.rename(columns={"lon": "gpsLon"}, inplace=True)
if "gpsLat" in df.columns:
    df.rename(columns={"gpsLat": "gnssLat"}, inplace=True)
if "gpsLon" in df.columns:
    df.rename(columns={"gpsLon": "gnssLon"}, inplace=True)
...
for i in range(stopPosn):
    if i == 0:
        ...
        accel.at[i, "accelVel"] = (
            accel.at[i - 1, "accelVel"]
            + (accel.at[i, "accelZ_Conv"] - 1)
            * 9.80665
            * (accel.at[i, "timeStamp"] - accel.at[i - 1, "timeStamp"])
            / 1000000
        )
        accel.at[i, "highGvel"] = (
            accel.at[i - 1, "highGvel"]
            + (accel.at[i, "highGz"] * 0.049 - 1)
            * 9.80665
            * (accel.at[i, "timeStamp"] - accel.at[i - 1, "timeStamp"])
            / 1000000
        )
...
for i in range(apogee):
    ...
    if vel.at[i, "baroVel"] > peakBaroVel:
        peakBaroVel = vel.at[i, "baroVel"]

    if i == 0:
        vel.at[i, "fusionVel2"] = 0

    else:
        vel.at[i, "fusionVel2"] = vel.at[i - 1, "fusionVel2"] + (
            vel.at[i, "accelZ"] * accelGain - 1
        ) * 9.80665 * (vel.at[i, "flightTime"] - vel.at[i - 1, "flightTime"])
    if (
        pd.notnull(vel.at[i, "baroVel"])
        and vel.at[i, "baroVel"] < vel.at[i, "fusionVel2"]
        and vel.at[i, "fusionVel2"] < 300
        and vel.at[i, "baroVel"] < peakBaroVel
        and vel.at[i, "accelZ"] * accelGain < 0.2
    ):
        vel.at[i, "fusionVel2"] = (
            vel.at[i, "fusionVel2"] * 0.99 + vel.at[i, "baroVel"] * 0.01
        )

    if i > 0 and vel.at[i, "gnssAlt"] != vel.at[i - 1, "gnssAlt"]:
        lastGNSSposn = i

for i in range(apogee, df.index.max()):

    if pd.notnull(vel.at[i, "baroVel"]):
        vel.at[i, "fusionVel2"] = vel.at[i, "baroVel"]
    else:
        vel.at[i, "fusionVel2"] = vel.at[i - 1, "fusionVel2"]

    if vel.at[i, "gnssAlt"] != vel.at[i - 1, "gnssAlt"]:
        vel.at[i, "fusionVel2"] = (
            vel.at[i, "gnssAlt"] - vel.at[lastGNSSposn, "gnssAlt"]
        ) / (vel.at[i, "flightTime"] - vel.at[lastGNSSposn, "flightTime"]) * 1 + vel.at[
            i - 1, "fusionVel2"
        ] * 0.0
        lastGNSSposn = i
...
-------------------------------"""
if df.at[0, "gnssAlt"] != 0:
    df["gnssAlt"] -= df.at[0, "gnssAlt"]
alt = df.loc[
    :,
    [
        "timeStamp",
...
for i in range(apogee):
    ...
    """Update Velocity"""
    if i == 0:
        alt.at[i, "fusionVel"] = 0
    else:
        alt.at[i, "fusionVel"] = (
            alt.at[i - 1, "fusionVel"]
            + (alt.at[i, "accelZ"] * accelGain - 1) * 9.80665 * dt
        )

    if (
        pd.notnull(alt.at[i, "baroVel"])
        and alt.at[i, "fusionVel"] < 300
        and i > 0
        and alt.at[i, "accelZ"] * accelGain < 0.2
    ):
        alt.at[i, "fusionVel"] = (
            alt.at[i - 1, "fusionVel"]
            + (alt.at[i, "accelZ"] * accelGain - 1) * 9.80665 * dt
        ) * 0.99 + alt.at[i, "baroVel"] * 0.01

    """Update Altitude"""
    if i == 0:
        alt.at[i, "fusionAlt"] = 0
    else:
        alt.at[i, "fusionAlt"] = (
            alt.at[i - 1, "fusionAlt"] + alt.at[i, "fusionVel"] * dt
        )

    if pd.notnull(alt.at[i, "altMoveAvg"]) and i > 0:
        alt.at[i, "fusionAlt"] = (
            alt.at[i - 1, "fusionAlt"] * 0.95 + alt.at[i, "altMoveAvg"] * 0.05
        )
...
gph = df.plot(
    ...
    ylabel="South-North",
    xlabel="West-East",
).annotate("Apogee", (df.at[apogee, "latitude"], df.at[apogee, "longitude"]))
...
for i in range(1, apogee, 1):
    ...
    origRotn.at[i, "dz"] = (
        origRotn.at[i - 1, "dz"] + origRotn.at[i, "gyroZ"] * origRotn.at[i, "dt"]
    )
    ...
    origRotn.at[i, "dx"] = (
        origRotn.at[i - 1, "dx"]
        + (cosZ * origRotn.at[i, "gyroX"] - sinZ * origRotn.at[i, "gyroY"])
        * origRotn.at[i, "dt"]
    )
    origRotn.at[i, "dy"] = (
        origRotn.at[i - 1, "dy"]
        + (cosZ * origRotn.at[i, "gyroY"] + sinZ * origRotn.at[i, "gyroX"])
        * origRotn.at[i, "dt"]
    )
...
origRotn.head(apogee).plot(
    x="flightTime",
    y=["pitchX", "yawY", "offVert"],
    ylabel="DCM2D Off-Vertical Rotation",
    xlabel="Flight Time",
)
...
quatLen = pow(
    quatRotn.at[0, "Quat1"] ** 2
    + quatRotn.at[0, "Quat2"] ** 2
    + quatRotn.at[0, "Quat3"] ** 2
    + quatRotn.at[0, "Quat4"] ** 2,
    -0.5,
)
...
for i in range(1, apogee, 1):
    ...
    if quatRotn.at[i, "timeStamp"] - lastquatRotn > 10000:
        ...
        quatRotn.at[i, "QuatDiff1"] = 0.5 * (
            -1 * dx * quatRotn.at[i - 1, "Quat2"]
            - dy * quatRotn.at[i - 1, "Quat3"]
            - dz * quatRotn.at[i - 1, "Quat4"]
        )
        quatRotn.at[i, "QuatDiff2"] = 0.5 * (
            dx * quatRotn.at[i - 1, "Quat1"]
            - dy * quatRotn.at[i - 1, "Quat4"]
            + dz * quatRotn.at[i - 1, "Quat3"]
        )
        quatRotn.at[i, "QuatDiff3"] = 0.5 * (
            dx * quatRotn.at[i - 1, "Quat4"]
            + dy * quatRotn.at[i - 1, "Quat1"]
            - dz * quatRotn.at[i - 1, "Quat2"]
        )
        quatRotn.at[i, "QuatDiff4"] = 0.5 * (
            -1 * dx * quatRotn.at[i - 1, "Quat3"]
            + dy * quatRotn.at[i - 1, "Quat2"]
            + dz * quatRotn.at[i - 1, "Quat1"]
        )

        """Update the quaternion"""
        quatRotn.at[i, "Quat1"] = (
            quatRotn.at[i - 1, "Quat1"] + quatRotn.at[i, "QuatDiff1"]
        )
        quatRotn.at[i, "Quat2"] = (
            quatRotn.at[i - 1, "Quat2"] + quatRotn.at[i, "QuatDiff2"]
        )
        quatRotn.at[i, "Quat3"] = (
            quatRotn.at[i - 1, "Quat3"] + quatRotn.at[i, "QuatDiff3"]
        )
        quatRotn.at[i, "Quat4"] = (
            quatRotn.at[i - 1, "Quat4"] + quatRotn.at[i, "QuatDiff4"]
        )

        """re-normalize"""
        quatLen = pow(
            pow(quatRotn.at[i, "Quat1"], 2)
            + pow(quatRotn.at[i, "Quat2"], 2)
            + pow(quatRotn.at[i, "Quat3"], 2)
            + pow(quatRotn.at[i, "Quat4"], 2),
            -0.5,
        )
...
quatRotn.head(apogee).plot(
    x="flightTime",
    y=["pitchX", "yawY", "offVert"],
    ylabel="Quaternion Rotation (degrees)",
)
...
rotnCompare["VertQuat-DCM2D"] = (
    rotnCompare["offVert_Quat"] - rotnCompare["offVert_DCM2D"]
)
...
if magApogee >= mag.index.max() - 10:
    magApogee = mag[mag["fltEvents"].str[6] == "1"].index[0]

# magXoffset = stats.mean(mag['magX'].head(200))
# magYoffset = stats.mean(mag['magY'].head(200))
magXoffset = (
    min(mag["magX"].head(200))
    + (max(mag["magX"].head(200)) - min(mag["magX"].head(200))) / 2
)
magYoffset = (
    min(mag["magY"].head(200))
    + (max(mag["magY"].head(200)) - min(mag["magY"].head(200))) / 2
)
...
mag.head(magApogee).plot(
    x="timeStamp", y=["magX_corrected", "magY_corrected", "magZ_corrected"]
)
...
for i in range(1, magApogee, 1):
    ...
    mag.at[i, "magYaw"] = (
        mag.at[i, "magOffVert"] * math.sqrt(1 - cosMagRoll**2) * 180 / math.pi
    )
...
mag.head(magApogee).plot(
    x="flightTime", y="magRoll", title="Magnetometer Calculated Roll"
)
...
magCompare["offVert"] *= 0.1
magCompare["pitchX"] *= 0.1
magCompare["yawY"] *= 0.1
...
for i in range(0, magCompare.index.max(), 1):
    magCompare2.at[i, "cosDCM2DRoll"] = math.cos(
        magCompare2.at[i, "rollZ_DCM2D"] * math.pi / 180
    )
...
for i in range(0, magApogee, 1):
    magCompare2.at[i, "cosDiff"] = abs(
        magCompare2.at[i, "cosDCM2DRoll"] - magCompare2.at[i, "cosMagRoll"]
    )
