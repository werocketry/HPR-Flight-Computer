...
for i in range(df.timeStamp.index.max()):
    ...
    if i == 0:
        ...
        df.at[i, "dt"] = df.at[i, "timeStamp"] - df.at[i - 1, "timeStamp"]
...
# plot gps packets
df.fillna(method="pad").plot(
    y="gnssSatellites", x="flightTime", title="GNSS Coords RXd"
)

# plot telemetry packets
for i in range(df.timeStamp.index.max()):
    if pd.isna(df.at[i, "radioPacketNum"]):
        df.at[i, "packetNum"] = 0
    else:
        df.at[i, "packetNum"] = 1
df.plot(y="packetNum", x="flightTime", title="Packets Sent", legend=False)
