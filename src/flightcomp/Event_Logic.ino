/**
 * @file Event_Logic.ino
 * @brief Main event logic for rocket control. Manages the firing of pyros and
 * event handling during the flight.
 * 
 * This logic is modular to facilitate maintenance and is the central control point
 * for the rocket's operations during its various flight stages.
 * 
 * @remark Change log:
 * - 17 JUL 21: Initial breakout created
 * - 19 SEP 23: Removed accelerometer-based apogee detection due to early deployments
 */


/**
 * @brief Check and manage rocket events during the flight.
 * 
 * This function manages the rocket's key events, including timeouts, false liftoff checks, 
 * booster burnout, stage separations, and parachute deployments. It also handles touch down 
 * detection and pyrotechnic control.
 */
void checkEvents()
{
  ...
  if (!events.timeOut && !pyroFire && fltTime.timeCurrent > settings.rcdTime)
  {
    ...
    {
      radio.event = Booster_Time_Limit;
    }
  ...
  {
    if (fltTime.timeCurrent > fltTime.detectLiftoffTime)
    {
    ...
    if (accel.z < gTrigger && accelVel < thresholdVel)
    {
      ...
      filterPosn = 0;
      highGsum = 0;
      for (byte i = 0; i < (sizeof(highGfilter) / sizeof(highGfilter[0])); i++)
      {
        highGfilter[i] = 0;
      }
      filterFull = false;
  ...
  if (!events.boosterBurnout && events.liftoff && accel.z <= 0)
  {
    ...
    if (settings.inflightRecover != 0 && !settings.testMode)
    {
      EEPROM.update(eeprom.lastEvent, radio.event);
    }
  ...
  {
    if (fltTime.timeCurrent - fltTime.boosterBurnout > boosterBurpTime)
    {
  ...
  if (settings.fltProfile == 'A')
  {
    ...
    {
      if (!events.airStart1BurnoutCheck && !events.apogee && !events.airStart1Burnout && accelNow < 0.0)
      {
        events.airStart1BurnoutCheck = true;
        fltTime.airStart1Burnout = fltTime.timeCurrent;
      }
      if (events.airStart1BurnoutCheck && fltTime.timeCurrent < (fltTime.airStart1Burnout + boosterBurpTime) && accel.z > 0)
      {
        events.airStart1BurnoutCheck = false;
      }
      if (events.airStart1BurnoutCheck && fltTime.timeCurrent > (fltTime.airStart1Burnout + boosterBurpTime))
      {
        events.airStart1BurnoutCheck = false;
        events.airStart1Burnout = true;
    ...
    {
      if (!events.airStart2BurnoutCheck && !events.apogee && !events.airStart2Burnout && accelNow < 0.0)
      {
        events.airStart2BurnoutCheck = true;
        fltTime.airStart2Burnout = fltTime.timeCurrent;
      }
      if (events.airStart2BurnoutCheck && fltTime.timeCurrent < (fltTime.airStart2Burnout + boosterBurpTime) && accel.z > 0)
      {
  ...
  if (!events.apogeeFire && events.apogee && fltTime.timeCurrent - fltTime.apogee >= settings.apogeeDelay)
  {
    ...
    if (settings.fltProfile == 'B')
    {
      radio.event = Fire_Booster_Apogee;
    }
    if (settings.inflightRecover != 0 && !settings.testMode)
    {
      EEPROM.update(eeprom.lastEvent, radio.event);
  ...
  if (events.apogeeFire && !syncApogee && !settings.testMode && fltTime.timeCurrent - fltTime.apogeeFire >= 3000000UL)
  {
    syncSD();
    syncApogee = true;
  }
  ...
  if (events.apogeeFire && !events.mainDeploy && accel.z > 4 * g && fltTime.timeCurrent - fltTime.apogeeFire <= 2000000UL)
  {
    ...
    if (settings.fltProfile == 'B')
    {
      radio.event = Booster_Separation;
    }
    if (settings.inflightRecover != 0 && !settings.testMode)
    {
      EEPROM.update(eeprom.lastEvent, radio.event);
  ...
  if (!events.mainDeploy && events.apogeeFire && baro.Alt < settings.mainDeployAlt && fltTime.timeCurrent - fltTime.apogeeFire >= 1000000UL)
  {
    ...
    if (settings.fltProfile == 'B')
    {
      radio.event = Fire_Booster_Mains;
    }
    if (settings.inflightRecover != 0 && !settings.testMode)
    {
      EEPROM.update(eeprom.lastEvent, radio.event);
  ...
  if (events.mainDeploy && fltTime.timeCurrent - fltTime.mainDeploy > 50000UL && fltTime.timeCurrent - fltTime.mainDeploy < 3000000UL && accelNow > 50.0)
  {
    ...
    if (settings.fltProfile == 'B')
    {
      radio.event = Booster_Under_Chute;
    }
    if (settings.inflightRecover != 0 && !settings.testMode)
    {
      EEPROM.update(eeprom.lastEvent, radio.event);
  ...
  if (events.mainDeploy && !syncMains && !settings.testMode && fltTime.timeCurrent - fltTime.mainDeploy >= 3000000UL)
  {
    syncSD();
    syncMains = true;
  }
  ...
  if (pyroFire)
  {
    ...
    // Turn off the pyros
    if (pyro1.fireStatus && fltTime.timeCurrent - pyro1.fireStart > settings.fireTime)
    {
      digitalWrite(pyro1.firePin, LOW);
      pyro1.fireStatus = false;
    }
    if (pyro2.fireStatus && fltTime.timeCurrent - pyro2.fireStart > settings.fireTime)
    {
      digitalWrite(pyro2.firePin, LOW);
      pyro2.fireStatus = false;
    }
    if (pyro3.fireStatus && fltTime.timeCurrent - pyro3.fireStart > settings.fireTime)
    {
      digitalWrite(pyro3.firePin, LOW);
      pyro3.fireStatus = false;
    }
    if (pyro4.fireStatus && fltTime.timeCurrent - pyro4.fireStart > settings.fireTime)
    {
      digitalWrite(pyro4.firePin, LOW);
      pyro4.fireStatus = false;
    }
    if (!pyro1.fireStatus && !pyro2.fireStatus && !pyro3.fireStatus && !pyro4.fireStatus)
    {
      pyroFire = false;
    }
  }
