/**
 * @file hpr_flightcomp_V4_6_0.cpp
 * @brief High-Power Rocketry Flight Computer for Teensy 3.2, 3.5, 3.6, 4.0, and 4.1
 * 
 * Original sketch by Bryan Sparkman, TRA #12111, NAR #85720, L3
 * Modified and continued by Cameron K. Brooks
 * 
 *
 * @section features Features
 * - Dual-deploy flight computer capable to over 100,000ft
 * - Two-stage & airstart capable with tilt-sensing safety features
 * - Live telemetry over 433MHz or 915MHz LoRa 
 *   - 433MHz: USA amateur 70cm band, EUR license-free 
 *   - 915MHz: USA FHSS license-free or USA amateur license use non-FHSS
 ...
 *   - 1000Hz of sensor-fused speed & altitude
 *   - 100Hz of roll, pitch, yaw rotation
 *   - 40Hz of magnetic data logging and magnetic roll
 *   - 30Hz-100Hz of digital barometric data logging 
 ...
 * - Mount in any orientation with automatic orientation detection during calibration
 * - Bench-test mode activated with tactile button
 ...
 * - V4_0_0 combines all previous versions coded for specific hardware setups
 * - V4_1_0 adds upgrades for airstart capability, more flight event codes, improved settings file, PWM Pyro firing, quaternion rotation
 * - V4_1_1 eliminates timer interrupts, improves telemetry timing, enables the Galileo GNSS system
 * - V4_1_2 increases the GPS data rates and fixes some bugs in the pyro firing timing
 * - V4_1_3 adds features to the settings file, moves some settings to the EEPROM file, adds calibration for the MPL3115A2
 * - V4_1_4 adds magnetic rotation and increases the magnetic data capture rate
 ...
 * - V4_2_3 adds inflight recovery from brown-out or MCU reset, sensor fusion altitude and velocity, eliminates gyro high-pass filter
 * - V4_3_0 updates Quaternion rotation for the best possible precision, corrects some GNSS bugs
 * - V4_3_1 makes FHSS an option on 915MHz and corrects some bugs in the FHSS power settings code
 * - V4_3_2 fixes bugs in the orientation & rotation routines, forces inline functions for faster execution, fixes multiple bugs picked up by the new Arduino compiler
 * - V4_3_3 creates a user interface over serial to calibrate the barometer, extends gyro orientation integration through the entire flight
 * - V4_3_4 enables active stabilization, creates basic fin trim code, and enables auto-adjusting gain on the IMU accelerometer
 * - V4_3_5 removes auto-adjusting gain, improves calibration routines, extends baro calibration to all sensors, removed high-G axis mode from user settings
 * - V4_3_6 adds return-to-base functionality (beta testing), supports LSM6DS33, LIS3MDL, and MS5607, updates code for separate mag sensors
 * - V4_4_0 creates flexible I2C and SPI bus routines for various devices and buses across Teensy3.X and Teensy4.X platforms
 ...
 * - V4_5_0 eliminates RadioHead library due to interferences with the PWMServo library, now uses an interval timer for precise control of the radio packet timing
 * - V4_5_1 adds optional GPS output file to capture NMEA sentences for contest flights, creates better radio resiliency by adding the callsign back into the header of the packet
 * - V4_5_2 finishes the draft RTB code, adds a canard calibration flight mode, fixed GPS bug with NEO-M8N
 * - V4_5_3 uses the RFM96W in both 70cm and 900MHz mode based on user frequency input for 3 options
 * - V4_5_4 fixes a bug in the sensor timing that reduced the effective data capture rate
 * - V4_5_5 removes launch detection user options since the algorithm is proven reliable, fixes a timing bug with the barometers, makes GPS configuration code more portable, adds LPS25H support, fixes bugs with LSM6DS33 and LIS3MDL
 * - V4_6_0 improves portability of quaternion rotation code, adds MPU6050 support, fixed calibration routine bug, cleaned up some of the GPS data processing, corrected major bug in high-G moving average
 *
 * @section future_upgrades Future Upgrades
...
struct
{
  ...
  int baroBusType = 88;// High-Power Rocketry Flight Computer (TeensyFlight)
// Original sketch by Bryan Sparkman, TRA #12111, NAR #85720, L3
...
// stuck-in-a-loop detection and breakout
  ...
  int sdCSpin = 117;
  int baroCSpin = 118;
  int servo1pin = 119; // 119   : servo 1 control pin// High-Power Rocketry Flight Computer (TeensyFlight)
// Original sketch by Bryan Sparkman, TRA #12111, NAR #85720, L3
// Built for Teensy 3.2, 3.5, 3.6, 4.0, and 4.1
// Code Line Count: 9644 lines of code = 2430 MainFile + 354 Bus_Mgmt + 2282 SensorDrivers + 900 Calibration + 625 SpeedTrig + 471 Inflight_Recover + 683 SD + 554 Rotation + 726 Telemetry + 327 Event_Logic + 292 GPSconfig
//--------FEATURES----------
...
/*!
 * @brief Initializes the flight computer setup.
 * 
 * This function configures the system by initializing sensors, setting global variables, 
 * and reading configurations from EEPROM.
 */
void setup(void)
{
  ...
  if (settings.inflightRecover > 0 && (lastEvent < 24 && lastEvent != 6 && lastEvent != 7))
  {
    ...
    Serial.print("Rapid Reset Initiated, setting: ");
    Serial.println(settings.inflightRecover);
    if (rapidReset())
    {
  ...
  for (byte i = 0; i < sizeof(settings.callSign); i++)
  {
    settings.callSign[i] = EEPROM.read(eeprom.callSign + i);
  }
  ...
  if (pins.radioEN != pins.nullCont)
  {
    pinMode(pins.radioEN, OUTPUT);
    digitalWrite(pins.radioEN, HIGH);
  }
  // start the radio
  ...
  if (sensors.status_RFM96W)
  {
    Serial.println(F("RFM96W/95W OK!"));
  }
  else
  {
    Serial.println(F("RFM96W/95W not found!"));
  }

  // safety override of user settings
  if (settings.apogeeDelay > 5000000UL)
  {
    settings.apogeeDelay = 5000000UL;
  } // 5s max apogee delay
  if (settings.fireTime > 1000000UL)
  {
    settings.fireTime = 1000000UL;
  } // 1s max firing length
  if (settings.mainDeployAlt > 458)
  {
    settings.mainDeployAlt = 458;
  } // max of 1500 ft
  if (settings.mainDeployAlt < 30)
  {
    settings.mainDeployAlt = 30;
  } // minimum of 100ft
  if (settings.sustainerFireDelay > 8000000UL)
  {
    settings.sustainerFireDelay = 8000000UL;
  } // maximum 8s 2nd stage ignition delay
  if (settings.boosterSeparationDelay > 3000000UL)
  {
    settings.boosterSeparationDelay = 3000000UL;
  } // max 3s booster separation delay after burnout
  if (settings.airStart1Delay > 2000000UL)
  {
    settings.airStart1Delay = 2000000UL;
  } // max 2s airstart delay
  if (settings.airStart2Delay > 2000000UL)
  {
    settings.airStart2Delay = 2000000UL;
  } // max 2s airstart delay
  if (settings.altThreshold < 91)
  {
    settings.altThreshold = 91;
  } // minimum 100ft threshold
  if (settings.maxAngle > 450)
  {
    settings.maxAngle = 450;
  } // maximum 45 degree off vertical
  if (settings.rcdTime < 300000000UL)
  {
    settings.rcdTime = 300000000UL;
  } // min 5min of recording time
  if (settings.fireTime < 200000UL)
  {
    settings.fireTime = 200000UL;
  } // min 0.2s of firing time
  if (settings.fireTime > 1000000UL)
  {
    settings.fireTime = 1000000UL;
  } // max 1.0s of firing time
  if (settings.setupTime > 60000UL)
  {
    settings.setupTime = 60000UL;
  } // max 60 seconds from power-on to preflight start
  if (settings.setupTime < 3000UL)
  {
    settings.setupTime = 3000UL;
  } // min 3 seconds of setup time
  if (settings.TXpwr > 20)
  {
    settings.TXpwr = 20;
  }
  if (settings.TXpwr < 2)
  ...
  if (settings.FHSS && settings.TXfreq < 900)
  {
    settings.FHSS = false;
  } // FHSS not used on 70cm band
  if (settings.fltProfile == '2' or settings.fltProfile == 'A')
  {
  ...
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.setupTime + i, ulongUnion.Byte[i]);
  }
  floatUnion.val = settings.mainDeployAlt;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.mainDeployAlt + i, floatUnion.Byte[i]);
  }
  ulongUnion.val = settings.apogeeDelay;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.apogeeDelay + i, ulongUnion.Byte[i]);
  }
  ulongUnion.val = settings.rcdTime;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.rcdTime + i, ulongUnion.Byte[i]);
  }
  ...
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.fireTime + i, ulongUnion.Byte[i]);
  }
  ...
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.TXfreq + i, floatUnion.Byte[i]);
  }
  ulongUnion.val = settings.boosterSeparationDelay;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.boosterSeparationDelay + i, ulongUnion.Byte[i]);
  }
  ulongUnion.val = settings.sustainerFireDelay;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.sustainerFireDelay + i, ulongUnion.Byte[i]);
  }
  EEPROM.update(eeprom.airStart1Event, settings.airStart1Event);
  ulongUnion.val = settings.airStart1Delay;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.airStart1Delay + i, ulongUnion.Byte[i]);
  }
  EEPROM.update(eeprom.airStart2Event, settings.airStart2Event);
  ulongUnion.val = settings.airStart2Delay;
  for (byte i = 0; i < 4; i++)
  {
    EEPROM.update(eeprom.airStart2Delay + i, ulongUnion.Byte[i]);
  }
  intUnion.val = settings.altThreshold;
  for (byte i = 0; i < 2; i++)
  {
    EEPROM.update(eeprom.altThreshold + i, intUnion.Byte[i]);
  }
  intUnion.val = settings.maxAngle;
  for (byte i = 0; i < 2; i++)
  {
    EEPROM.update(eeprom.maxAngle + i, intUnion.Byte[i]);
  }
  ...
  pyro1.func = settings.pyro1Func;
  pyro1.contPin = pins.pyro1Cont;
  pyro1.firePin = pins.pyro1Fire;
  pyro1.fireStatus = false;
  pyro1.fireStart = 0UL;
  pyro2.func = settings.pyro2Func;
  pyro2.contPin = pins.pyro2Cont;
  pyro2.firePin = pins.pyro2Fire;
  pyro2.fireStatus = false;
  pyro2.fireStart = 0UL;
  pyro3.func = settings.pyro3Func;
  pyro3.contPin = pins.pyro3Cont;
  pyro3.firePin = pins.pyro3Fire;
  pyro3.fireStatus = false;
  pyro3.fireStart = 0UL;
  pyro4.func = settings.pyro4Func;
  pyro4.contPin = pins.pyro4Cont;
  pyro4.firePin = pins.pyro4Fire;
  pyro4.fireStatus = false;
  pyro4.fireStart = 0UL;

  if (settings.testMode)
  {
    Serial.print(F("Flight Profile: "));
    Serial.println(settings.fltProfile);
    Serial.print(F("Pyro 4 Function: "));
    Serial.println(pyro4.func);
    Serial.print(F("Pyro 3 Function: "));
    Serial.println(pyro3.func);
    Serial.print(F("Pyro 2 Function: "));
    Serial.println(pyro2.func);
    Serial.print(F("Pyro 1 Function: "));
    Serial.println(pyro1.func);
  }

  // check for silent mode
  if (settings.testMode && settings.silentMode)
  {
    pins.beep = pins.nullCont;
    Serial.println(F("Silent Mode Confirmed"));
  }
  ...
  if (settings.TXenable)
  {
    ...
    if (settings.TXfreq > 900.000F && !settings.FHSS)
    {
      ...
      if (settings.testMode)
      {
        Serial.println(F("Dedicated ISM band frequency. Power limit 2mW"));
      }
    }
    ...
    if (settings.testMode)
    {
      ...
      Serial.println(settings.TXfreq, 3);
  ...
  if (settings.stableRotn || settings.stableVert)
  {
    ...
    if (settings.testMode)
    {
      Serial.println(F("ACTIVE STABILIZATION ENABLED"));
    }
    ...
    if (settings.testMode)
    {
      Serial.print(F("Servo1 Trim: "));
      Serial.println(servo1trim);
      Serial.print(F("Servo2 Trim: "));
      Serial.println(servo2trim);
      Serial.print(F("Servo3 Trim: "));
      Serial.println(servo3trim);
      Serial.print(F("Servo4 Trim: "));
      Serial.println(servo4trim);
    }
  ...
  if (settings.testMode)
  {
    ...
    while (beep_counter < 8)
    {
      ...
      if (digitalRead(pins.testRead) == HIGH)
      {
  ...
  if (settings.magSwitchEnable == 1)
  {
    resetMagGain();
  }

  // read the bias from EEPROM
  calUnion.calByte[0] = EEPROM.read(eeprom.accelBiasX);
  calUnion.calByte[1] = EEPROM.read(eeprom.accelBiasX + 1);
  accel.biasX = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.accelBiasY);
  calUnion.calByte[1] = EEPROM.read(eeprom.accelBiasY + 1);
  accel.biasY = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.accelBiasZ);
  calUnion.calByte[1] = EEPROM.read(eeprom.accelBiasZ + 1);
  accel.biasZ = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.highGbiasX);
  calUnion.calByte[1] = EEPROM.read(eeprom.highGbiasX + 1);
  highG.biasX = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.highGbiasY);
  calUnion.calByte[1] = EEPROM.read(eeprom.highGbiasY + 1);
  highG.biasY = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.highGbiasZ);
  calUnion.calByte[1] = EEPROM.read(eeprom.highGbiasZ + 1);
  highG.biasZ = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.magBiasX);
  calUnion.calByte[1] = EEPROM.read(eeprom.magBiasX + 1);
  mag.biasX = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.magBiasY);
  calUnion.calByte[1] = EEPROM.read(eeprom.magBiasY + 1);
  mag.biasY = calUnion.calValue;
  calUnion.calByte[0] = EEPROM.read(eeprom.magBiasZ);
  calUnion.calByte[1] = EEPROM.read(eeprom.magBiasZ + 1);
  mag.biasZ = calUnion.calValue;
  for (byte i = 0; i < 4; i++)
  {
    floatUnion.Byte[i] = (byte)EEPROM.read(eeprom.baroPressOffset + i);
  }
  baro.pressOffset = floatUnion.val;
  for (byte i = 0; i < 4; i++)
  {
    floatUnion.Byte[i] = (byte)EEPROM.read(eeprom.baroTempOffset + i);
  }
  ...
  for (byte i = 0; i < 4; i++)
  {
    if ((byte)EEPROM.read(eeprom.baroPressOffset + i) == (byte)255)
    {
      unusedBytes++;
    }
  }
  if (unusedBytes >= 4)
  {
    if (settings.testMode)
    {
      Serial.println(F("Warning: Barometer is not calibrated!"));
    }
  ...
  for (byte i = 0; i < 2; i++)
  {
    if ((byte)EEPROM.read(eeprom.accelBiasX + i) == (byte)255)
    {
      unusedBytes++;
    }
  }
  if (unusedBytes >= 2)
  {
    if (settings.testMode)
    {
      Serial.println(F("Warning: Accelerometer is not calibrated!"));
    }
  ...
  if (unusedBytes >= 2 && !sensors.status_NoHighGAccel)
  {
    if (settings.testMode)
    {
      Serial.println(F("Warning: High-G Accelerometer is not calibrated!"));
    }
  ...
  for (byte i = 0; i < 2; i++)
  {
    if ((byte)EEPROM.read(eeprom.magBiasX + i) == (byte)255)
    {
      unusedBytes++;
    }
  }
  if (unusedBytes >= 2)
  {
    if (settings.testMode)
    {
      Serial.println(F("Warning: Magnetometer is not calibrated!"));
    }
  ...
  if (settings.testMode)
  {
    ...
    Serial.print(F("accel.biasX: "));
    Serial.println(accel.biasX);
    Serial.print(F("accel.biasY: "));
    Serial.println(accel.biasY);
    Serial.print(F("accel.biasZ: "));
    Serial.println(accel.biasZ);
    Serial.print(F("highG.biasX: "));
    Serial.println(highG.biasX);
    Serial.print(F("highG.biasY: "));
    Serial.println(highG.biasY);
    Serial.print(F("highG.biasZ: "));
    Serial.println(highG.biasZ);
    Serial.print(F("mag.biasX: "));
    Serial.println(mag.biasX);
    Serial.print(F("mag.biasY: "));
    Serial.println(mag.biasY);
    Serial.print(F("mag.biasZ: "));
    Serial.println(mag.biasZ);
    Serial.print(F("baro.tempOffset: "));
    Serial.println(baro.tempOffset, 1);
    Serial.print(F("baro.pressOffset: "));
    Serial.println(baro.pressOffset, 2);
  }
  ...
  if (settings.testMode)
  {
    Serial.print(F("accel.X is pointed to real world: "));
    Serial.print((accel.dirX == 1) ? '+' : '-');
    Serial.println(accel.orientX);
    Serial.print(F("accel.Y is pointed to real world: "));
    Serial.print((accel.dirY == 1) ? '+' : '-');
    Serial.println(accel.orientY);
    Serial.print(F("accel.Z is pointed to real world: "));
    Serial.print((accel.dirZ == 1) ? '+' : '-');
    Serial.println(accel.orientZ);
    Serial.print(F("gyro.X is pointed to real world: "));
    Serial.print((gyro.dirX == 1) ? '+' : '-');
    Serial.println(gyro.orientX);
    Serial.print(F("gyro.Y is pointed to real world: "));
    Serial.print((gyro.dirY == 1) ? '+' : '-');
    Serial.println(gyro.orientY);
    Serial.print(F("gyro.Z is pointed to real world: "));
    Serial.print((gyro.dirZ == 1) ? '+' : '-');
    Serial.println(gyro.orientZ);
    Serial.print(F("mag.X is pointed to real world: "));
    Serial.print((mag.dirX == 1) ? '+' : '-');
    Serial.println(mag.orientX);
    Serial.print(F("mag.Y is pointed to real world: "));
    Serial.print((mag.dirY == 1) ? '+' : '-');
    Serial.println(mag.orientY);
    Serial.print(F("mag.Z is pointed to real world: "));
    Serial.print((mag.dirZ == 1) ? '+' : '-');
    Serial.println(mag.orientZ);
    Serial.print(F("highG.X is pointed to real world: "));
    Serial.print((highG.dirX == 1) ? '+' : '-');
    Serial.println(highG.orientX);
    Serial.print(F("highG.Y is pointed to real world: "));
    Serial.print((highG.dirY == 1) ? '+' : '-');
    Serial.println(highG.orientY);
    Serial.print(F("highG.Z is pointed to real world: "));
    Serial.print((highG.dirZ == 1) ? '+' : '-');
    Serial.println(highG.orientZ);
  }
  ...
  if (settings.testMode)
  {
    ...
    if (settings.stableRotn || settings.stableVert)
    {
      settings.rcdTime = 30000000UL;
    }                          // 30s of record time for serial plotting
  ...
  if (settings.testMode)
  {
    Serial.print("Pyro4 Continuity: ");
    Serial.println((pyro4.contStatus) ? "Y" : "N");
    Serial.print("Pyro3 Continuity: ");
    Serial.println((pyro3.contStatus) ? "Y" : "N");
    Serial.print("Pyro2 Continuity: ");
    Serial.println((pyro2.contStatus) ? "Y" : "N");
    Serial.print("Pyro1 Continuity: ");
    Serial.println((pyro1.contStatus) ? "Y" : "N");
    Serial.print(F("Reporting continuity code: "));
    Serial.println(cont.beepCode);
  }
  ...
  if (settings.magSwitchEnable)
  {
    ...
    while (!magDetect)
    {
      ...
      while (!magDetect && ii < beepCode)
      {
        ...
        delay(250);
        getMag();
        if (abs(mag.x) > magTrigger || abs(mag.y) > magTrigger || abs(mag.z) > magTrigger)
        {
          magDetect = true;
        }
        ii++;
  ...
  if (settings.testMode)
  {
    Serial.println(F("Hold Rocket Vertical"));
  }
  ...
  while (micros() - calibrationStart < sampTime)
  {
    ...
    if (settings.testMode && accelSamps % 100 == 0 && samplePrint)
    {
      ...
      Serial.print("HighG: ");
      Serial.print(highG.x);
      Serial.print(',');
      Serial.print(highG.y);
      Serial.print(',');
  ...
  Serial.println(gyroSamps);
  Serial.print("accel samples: ");
  Serial.println(accelSamps);
  Serial.print("highG samples: ");
  Serial.println(highGsamps);
  Serial.print("mag samples: ");
  Serial.println(magSamps);
  ...
  if (settings.testMode)
  {
    ...
    Serial.print(gyro.biasX);
    Serial.print(F(", "));
    Serial.print(gyro.biasY);
    Serial.print(F(", "));
    ...
    Serial.print(highG.x0);
    Serial.print(F(", "));
    Serial.print(highG.y0);
    Serial.print(F(", "));
  ...
  if (accel.y0 >= 0)
  {
    yawY0 = asinf(min(1, (float)accel.y0 / (float)g)) * rad2deg;
  }
  else
  {
    yawY0 = asinf(max(-1, (float)accel.y0 / (float)g)) * rad2deg;
  }

  if (accel.x0 >= 0)
  {
    pitchX0 = asinf(min(1, (float)accel.x0 / (float)g)) * rad2deg;
  }
  else
  {
    pitchX0 = asinf(max(-1, (float)accel.x0 / (float)g)) * rad2deg;
  }
  ...
  if (settings.stableRotn || settings.stableVert)
  {
    getDCM2DRotn(pitchX0 * 1000000 / gyro.gainZ, yawY0 * 1000000 / gyro.gainZ, 0, gyro.gainZ);
  }
  ...
  if (settings.testMode)
  {
    ...
    Serial.print(F("Yaw: "));
    Serial.println(yawY0, 2);
    Serial.print(F("Pitch: "));
    Serial.println(pitchX0, 2);
    Serial.print(F("Off Vertical: "));
    Serial.println(((float)offVert) * .1, 1);
  }
  ...
  {
    if (settings.pyro1Func == 'M')
    {
      pins.batt = pins.pyro1Cont;
    }
    if (settings.pyro2Func == 'M')
    {
      pins.batt = pins.pyro2Cont;
    }
    if (settings.pyro3Func == 'M')
    {
      pins.batt = pins.pyro3Cont;
    }
    voltReading = analogRead(pins.batt);
  ...
  if (accel.orientX == 'Z')
  {
    gTrigger -= accel.dirX * accel.biasX;
  }
  if (accel.orientY == 'Z')
  {
    gTrigger -= accel.dirY * accel.biasY;
  }
  if (accel.orientZ == 'Z')
  {
    gTrigger -= accel.dirZ * accel.biasZ;
  }
  ...
  if (settings.reportStyle == 'P')
  {
    ...
    if (settings.testMode)
    {
      Serial.print(F("Reporting Main Deploy Settings: "));
      Serial.println((int)(settings.mainDeployAlt * unitConvert));
    }
    // Beep out the main deployment altitude setting
    while (maxVelDigits[velDigits - 1] == 0)
    {
      velDigits--;
    }
  ...
  for (byte i = 0; i < altDigits; i++)
  {
    if (EEPROM.read(eeprom.maxFltAlt + i) != (byte)255)
    {
      initPgm = false;
    }
  }
  ...
  if (initPgm)
  {
    if (settings.testMode)
    {
      Serial.println(F("Initial Use Detected: Writing Initial EEPROM values for prior flight"));
    }
    for (byte j = 0; j < 6; j++)
    {
      EEPROM.update(eeprom.maxFltAlt + j, j);
    }
  }
  ...
  if (settings.reportStyle == 'P')
  {
    if (settings.testMode)
    {
      Serial.print(F("Reporting last flight: "));
    }
    for (byte i = 0; i < 6; i++)
    {
      maxAltDigits[i] = EEPROM.read(eeprom.maxFltAlt + i);
    }
    while (maxAltDigits[altDigits - 1] == 0 && altDigits > 0)
    {
      altDigits--;
    }
    for (byte i = altDigits; i > 0; i--)
    {
      ...
      if (settings.testMode)
      {
        if (maxAltDigits[i - 1] < 10)
        {
          Serial.print(maxAltDigits[i - 1]);
        }
        else
        {
          Serial.print('0');
  ...
  if (settings.reportStyle == 'P')
  {
    ...
    {
      Serial.println(" ");
      Serial.print(F("Reporting Battery Voltage: "));
  ...
  if (settings.fltProfile == 'B')
  {
    radio.event = Booster_Preflight;
  }
...
/*!
 * @brief Main flight loop for the flight computer.
 * 
 * This loop performs sensor sampling, event checking, and other critical tasks during rocket flight.
 */
void loop(void)
{
  ...
  if (sampleTimeCheck - accel.timeLastSamp > accel.timeBtwnSamp)
  {
    ...
    while (sampleTimeCheck - accel.timeLastSamp > accel.timeBtwnSamp)
    {
      accel.timeLastSamp += accel.timeBtwnSamp;
    }
  }
  ...
  if (sampleTimeCheck - gyro.timeLastSamp > gyro.timeBtwnSamp)
  {
    ...
    while (sampleTimeCheck - gyro.timeLastSamp > gyro.timeBtwnSamp)
    {
      gyro.timeLastSamp += gyro.timeBtwnSamp;
    }
  }
  ...
  if (sampleTimeCheck - highG.timeLastSamp > highG.timeBtwnSamp)
  {
    ...
    while (sampleTimeCheck - highG.timeLastSamp > highG.timeBtwnSamp)
    {
      highG.timeLastSamp += highG.timeBtwnSamp;
    }
  }
  ...
  if (sampleTimeCheck - mag.timeLastSamp > mag.timeBtwnSamp)
  {
    ...
    while (sampleTimeCheck - mag.timeLastSamp > mag.timeBtwnSamp)
    {
      mag.timeLastSamp += mag.timeBtwnSamp;
    }
  }
  ...
  if (!events.liftoff && accel.z > gTrigger && !events.touchdown && !events.timeOut)
  {
    ...
    {
      Serial.println(' ');
      Serial.println(F("Simulated Liftoff Detected!"));
    }
    ...
    if (settings.inflightRecover != 0)
    {
      for (byte i = 0; i < 4; i++)
      {
        EEPROM.update(eeprom.baseAlt + i, floatUnion.Byte[i]);
      }
  ...
  if (events.liftoff)
  {
    ...
    if (highG.newSamp)
    {
      ...
      if (!filterFull)
      {
        highGsmooth = highGsum / filterPosn;
      }
      else
      {
        highGsmooth = highGsum / sizeHighGfilter;
      }
    }

    // Compute the current g-load. Use the high-G accelerometer if the IMU is pegged and a high-G accelerometer is present
    if (abs(accel.z) < accel.ADCmax)
    {
      accelNow = (float)(accel.z - g) * accel.gainZ;
    }
    else if (!sensors.status_NoHighGAccel)
    {
      accelNow = (highG.z - (float)high1G) * highG.gainZ;
    }
    else
    {
      accelNow = (float)(accel.z - g) * accel.gainZ;
    ...
    if (!events.apogeeFire)
    {
      ...
      if (fusionVel > maxVelocity)
      {
        maxVelocity = fusionVel;
      }
      ...
      if (!altOK && (fusionAlt > settings.altThreshold || settings.testMode))
      {
        altOK = true;
      }
    ...
    if (settings.stableRotn || settings.stableVert || settings.flyBack)
    {
      ...
      if (controlTime - timeLastControl >= controlInterval)
      {
        ...
        if ((settings.stableVert || settings.stableRotn) && !events.apogee && events.boosterBurnout)
        {
          setCanards();
        }
        // if RTB is on and we are post apogee, then set canards to return to launch point
        if (settings.flyBack && events.apogeeFire && !events.mainDeploy)
        {
          setRTB();
        }
    ...
    if ((fltTime.timeCurrent - lastVolt > voltRate) || pyroFire)
    {
      ...
      {
        voltage *= (2.72 / 3.2);
      } // on old units we used to pull the battery voltage through one of the pyro channels which had diodes
      writeVolt = true;
    ...
    if (events.timeOut || events.touchdown)
    {
      ...
      while (maxAltDigits[altDigits - 1] == 0)
      {
        altDigits--;
      }
      while (maxVelDigits[velDigits - 1] == 0)
      ...
      if (!settings.testMode)
      {
        for (byte i = 0; i < 6; i++)
        {
          EEPROM.update(eeprom.maxFltAlt + i, maxAltDigits[i]);
        }
      }
      ...
      if (settings.testMode)
      {
        Serial.print(F("Max SD Write Time: "));
        Serial.println(maxWriteTime);
        Serial.print(F("Write Threshold: "));
        Serial.print(writeThreshold);
        Serial.print(F(", Count: "));
        Serial.println(writeThreshCount);
      }
  ...
  if (fileClose && beepVel && !beep && fltTime.tmClock - timeLastBeep > beep_delay)
  {
    ...
    if (beep_counter == beepCode)
    {
      ...
      if (beepPosn == velDigits)
      {
        beep_delay = 3000000UL;
      }
      beepPosn--;
  ...
  if (!gnss.configPwrSave && (events.touchdown || events.timeOut) && micros() - fltTime.touchdown > 5000000UL)
  {
    GPSpowerSaveMode(settings.testMode, sensors.GPS);
    gnss.configPwrSave = true;
  }
  // Read from serial
  ...
  while (HWSERIAL->available() > 0)
  {
    ...
    GPS.encode(c);
    if (settings.GPSlog && !fileClose)
    {
      updateGPSlogSD(c);
    }
    if (settings.testMode && GPSecho && !events.liftoff)
    {
      serialBuffer[serialPosn] = c;
      serialPosn++;
    }
  }
  if (settings.testMode && GPSecho && !events.liftoff && msgRX)
  {
  ...
  if (micros() - gnss.timeLastFix > 2000000UL && !events.postFlight)
  {
    gnss.fix = 0;
  }
  ...
  if (GPS.location.isUpdated() || GPS.altitude.isUpdated())
  {
    ...
    // capture max GPS alt
    if (radio.GPSalt > gnss.maxAlt)
    {
      gnss.maxAlt = radio.GPSalt;
    }
    ...
    gnss.altPosn++;
    if (gnss.altPosn >= (byte)(sizeof(gnss.altBuff) / sizeof(gnss.altBuff[0])))
    {
      gnss.altPosn = 0;
    }
    ...
    if (events.apogee)
    {
      fusionVel = 0.9 * gnss.vel + 0.1 * baro.Vel;
    }
    ...
    if (events.preLiftoff)
    {
      if (GPS.altitude.meters() != 0)
      {
        ...
        gnss.bufferPosn++;
        if (gnss.bufferPosn >= (byte)(sizeof(gnss.avgAlt) / sizeof(gnss.avgAlt[0])))
        {
          gnss.bufferPosn = 0;
          gnss.bufferFull = true;
        }
        ...
        pressurePosn++;
        if (pressurePosn >= (byte)(sizeof(pressureAvg5) / sizeof(pressureAvg5[0])))
        {
          pressurePosn = 0;
        }
        ...
        if (gnss.bufferFull)
        {
          baro.seaLevelPressure = pressureAvg / powf((44330 - gnss.baseAlt) / 44330, 5.254861);
        }
      }
...
void checkPyroContinuity()
{
  ...
  if (!events.liftoff)
  {
    if (pyro1.contStatus)
    {
      ...
      if (pyro1.func == 'I')
      {
        cont.upperStage = true;
      }
      if (pyro1.func == 'B')
      {
        cont.boosterSep = true;
      }
      if (pyro1.func == '1')
      {
        cont.airStart1 = true;
    ...
    if ((settings.fltProfile == '2' || settings.fltProfile == 'A') && !cont.upperStage && !cont.boosterSep && !cont.airStart1 && !cont.airStart2)
    {
      if (settings.testMode)
      {
        Serial.println(F("Complex Pyros Not Detected! Flight Profile set to single stage"));
      }
      settings.fltProfile = 'S';
      if (pyro1.func == 'I' || pyro1.func == 'B' || pyro1.func == '1' || pyro1.func == '2')
      {
        pyro1.func = 'N';
      }
      if (pyro2.func == 'I' || pyro2.func == 'B' || pyro2.func == '1' || pyro2.func == '2')
      {
        pyro2.func = 'N';
      }
      if (pyro3.func == 'I' || pyro3.func == 'B' || pyro3.func == '1' || pyro3.func == '2')
      {
        pyro3.func = 'N';
      }
      if (pyro4.func == 'I' || pyro4.func == 'B' || pyro4.func == '1' || pyro4.func == '2')
      {
        pyro4.func = 'N';
      }
    }
    ...
    if (!pyro1.contStatus && pyro1.func != 'N')
    {
      cont.error = true;
      cont.reportCode = cont.beepCode = 1;
    }
    if (!pyro2.contStatus && pyro2.func != 'N')
    {
      cont.error = true;
      cont.reportCode = cont.beepCode = 2;
    }
    if (!pyro3.contStatus && pyro3.func != 'N')
    {
      cont.error = true;
      cont.reportCode = cont.beepCode = 3;
    }
    if (!pyro4.contStatus && pyro4.func != 'N')
    {
      cont.error = true;
      cont.reportCode = cont.beepCode = 4;
    }
    if (!pyro1.contStatus && !pyro2.contStatus && !pyro3.contStatus && !pyro4.contStatus)
    {
      cont.reportCode = 0;
      cont.beepCode = 1;
    }
    ...
    {
      if (cont.boosterSep && cont.upperStage && cont.apogee && cont.main)
      {
        cont.beepCode = 4;
        cont.reportCode = 6;
      }
      else if (cont.upperStage && cont.apogee && cont.main)
      {
        cont.beepCode = 3;
        cont.reportCode = 5;
      }
    ...
    {
      if (cont.airStart1 && cont.airStart2 && cont.apogee && cont.main)
      {
        cont.beepCode = 4;
        cont.reportCode = 6;
      }
      else if (cont.airStart1 && cont.apogee && cont.main)
      {
        cont.beepCode = 3;
        cont.reportCode = 5;
      }
...
void firePyros(char event)
{
  ...
  if (pyro1.func == event)
  {
    pins.firePin = pyro1.firePin;
    digitalWrite(pyro1.firePin, HIGH);
    pyro1.fireStatus = true;
    pyro1.fireStart = fltTime.timeCurrent;
  }
  if (pyro2.func == event)
  {
    pins.firePin = pyro2.firePin;
    digitalWrite(pyro2.firePin, HIGH);
    pyro2.fireStatus = true;
    pyro2.fireStart = fltTime.timeCurrent;
  }
  if (pyro3.func == event)
  {
    pins.firePin = pyro3.firePin;
    digitalWrite(pyro3.firePin, HIGH);
    pyro3.fireStatus = true;
    pyro3.fireStart = fltTime.timeCurrent;
  }
  if (pyro4.func == event)
  {
    pins.firePin = pyro4.firePin;
    digitalWrite(pyro4.firePin, HIGH);
    pyro4.fireStatus = true;
    pyro4.fireStart = fltTime.timeCurrent;
  }
}
...
void readEEPROMsettings()
{
  ...
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.setupTime + i);
  }
  settings.setupTime = ulongUnion.val;
  for (byte i = 0; i < 4; i++)
  {
    floatUnion.Byte[i] = (byte)EEPROM.read(eeprom.mainDeployAlt + i);
  }
  settings.mainDeployAlt = floatUnion.val;
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.apogeeDelay + i);
  }
  settings.apogeeDelay = ulongUnion.val;
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.rcdTime + i);
  }
  ...
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.fireTime + i);
  }
  ...
  for (byte i = 0; i < 4; i++)
  {
    floatUnion.Byte[i] = (byte)EEPROM.read(eeprom.TXfreq + i);
  }
  settings.TXfreq = floatUnion.val;
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.boosterSeparationDelay + i);
  }
  settings.FHSS = (boolean)EEPROM.read(eeprom.FHSS);
  settings.boosterSeparationDelay = ulongUnion.val;
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.sustainerFireDelay + i);
  }
  settings.sustainerFireDelay = ulongUnion.val;
  settings.airStart1Event = (char)EEPROM.read(eeprom.airStart1Event);
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.airStart1Delay + i);
  }
  settings.airStart1Delay = ulongUnion.val;
  settings.airStart2Event = (char)EEPROM.read(eeprom.airStart2Event);
  for (byte i = 0; i < 4; i++)
  {
    ulongUnion.Byte[i] = (byte)EEPROM.read(eeprom.airStart2Delay + i);
  }
  settings.airStart2Delay = ulongUnion.val;
  for (byte i = 0; i < 2; i++)
  {
    intUnion.Byte[i] = (byte)EEPROM.read(eeprom.altThreshold + i);
  }
  settings.altThreshold = intUnion.val;
  for (byte i = 0; i < 2; i++)
  {
    intUnion.Byte[i] = (byte)EEPROM.read(eeprom.maxAngle + i);
  }
...
void processBaroSamp()
{
  ...
  if (events.preLiftoff || events.touchdown || events.timeOut)
  {
    ...
    if (baseAltPosn >= sizeBaseAltBuff)
    {
      baseAltPosn = 0;
    }
  ...
  if (baro.Alt > baro.maxAlt && !events.apogee)
  {
    baro.maxAlt = baro.Alt;
  }
  ...
  if (rawAltPosn >= (byte)sizeRawAltBuff)
  {
    rawAltPosn = 0;
  }
  ...
  else
  {
    ...
    if (baroVelPosn < 0)
    {
      baroVelPosn = (int)((sizeof(altAvgBuff) / sizeof(altAvgBuff[0])) - (10 - altAvgPosn));
    }
  }
  ...
  // update variables
  if (baro.Vel > baro.maxVel)
  {
    baro.maxVel = baro.Vel;
  }
  radio.vel = baro.Vel;
  ...
  altAvgPosn++;
  if (altAvgPosn >= (byte)(sizeof(altAvgBuff) / sizeof(altAvgBuff[0])))
  {
    altAvgPosn = 0;
  }
