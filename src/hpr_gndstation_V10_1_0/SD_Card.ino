...
void readRadioSettingsSD()
{
  ...
  settings.displayMetric = (boolean)(parseNextVariable(true));
  if (settings.displayMetric)
  {
    unitConvert = 1.0F;
  }
  ...
  {
    settings.callSign[i] = dataString[i];
  }
...
void createSustainerFile()
{
  ...
  while (SD.exists(sustainerFileName))
  {
    ...
    if (fileNum < 10)
    {
      itoa(fileNum, sustainerFileName + 7, 10);
    }
    else
    {
      itoa(fileNum, sustainerFileName + 6, 10);
    }
    sustainerFileName[8] = '.';
  }
  ...
  if (debugSerial)
  {
    ...
    Serial.println(sustainerFileName);
  }
  ...
  if (debugSerial)
  {
    Serial.println("Header Printed, Card Synced");
  }
}
...
void createBoosterFile()
{
  ...
  while (SD.exists(boosterFileName))
  {
    ...
    if (fileNum < 10)
    {
      itoa(fileNum, boosterFileName + 7, 10);
    }
    else
    {
      itoa(fileNum, boosterFileName + 6, 10);
    }
    boosterFileName[8] = '.';
  }
  ...
  {
    Serial.print("FileName: ");
    Serial.println(boosterFileName);
  }
  boosterFile.open(boosterFileName, FILE_WRITE);
  if (fileNum < 10)
  {
    itoa(fileNum, boosterFileName + 8, 10);
  }
  else
  {
    itoa(fileNum, boosterFileName + 7, 10);
...
void writePreflightData()
{
  strPosn = 0;
  writeIntData((int)0);
  for (byte i = 0; i < sizeof(rocketName) - 1; i++)
  {
    dataString[strPosn] = rocketName[i];
    strPosn++;
  }
  while (dataString[strPosn - 1] == ' ' || dataString[strPosn - 1] == '\0')
  {
    strPosn--;
  }
  dataString[strPosn] = ',';
  ...
  dataString[strPosn] = charGPSlat;
  strPosn++;
  writeFloatData(GPSlatitude, 4);
  dataString[strPosn] = charGPSlon;
  strPosn++;
  ...
  if (parseSustainer)
  {
    ...
    if (debugSerial)
    {
      Serial.println(F("Preflight Data Written"));
    }
  }
  if (parseBooster)
  {
...
void writeInflightData()
{
  ...
  // sync the SD card
  if (SDinit && parseSustainer)
  {
    sustainerFile.sync();
  }
  if (SDinit && parseBooster)
  {
    boosterFile.sync();
  }
}
...
void writePostflightData()
{
  ...
  if (parseSustainer)
  {
    sustainerFile.println(F("Rocket Name,lastEvent,maxAlt,maxVel,maxG,maxGPSalt,GPSlock,GPSalt,gpsLatitude,gpsLongitude"));
  }
  if (parseBooster)
  {
    boosterFile.println(F("Rocket Name,lastEvent,maxAlt,maxVel,maxG,maxGPSalt,GPSlock,GPSalt,gpsLatitude,gpsLongitude"));
  }
  for (byte i = 0; i < sizeof(rocketName) - 1; i++)
  {
    dataString[i] = rocketName[i];
    strPosn = i;
  }
  while (dataString[strPosn - 1] == ' ' || dataString[strPosn - 1] == '\0')
  {
  ...
  if (debugSerial)
  {
    Serial.println("Post Flight Data Written");
  }
}
