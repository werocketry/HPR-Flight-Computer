void preflightPacket(byte rxPacket[])
{
  ...
  pktPosn = eventPosn;
  event = (byte)rxPacket[pktPosn];
  pktPosn++; // 1
  GPSlock = (byte)rxPacket[pktPosn];
  pktPosn++; // 2
  contCode = (byte)rxPacket[pktPosn];
  pktPosn++; // 3
  for (byte i = 0; i < 20; i++)
  {
    rocketName[i] = (byte)rxPacket[pktPosn];
    pktPosn++;
  } // 23
  radioInt.unionByte[0] = (byte)rxPacket[pktPosn];
  pktPosn++; // 24
  radioInt.unionByte[1] = (byte)rxPacket[pktPosn];
  pktPosn++; // 25
  ...
  charGPSlat = (int8_t)rxPacket[pktPosn];
  pktPosn++; // 28
  for (byte i = 0; i < 4; i++)
  {
    radioUnion.unionByte[i] = (byte)rxPacket[pktPosn];
    pktPosn++;
  } // 32
...
void inflightPacket(byte rxPacket[])
{
  ...
  if (debugSerial)
  {
    Serial.print(F("Inflight Packet Received, PktNum: "));
  ...
  for (byte j = 0; j < 4; j++)
  {
    radioUnion.unionByte[j] = (byte)rxPacket[pktPosn];
    pktPosn++;
  } // 60
  GPSlatitude = radioUnion.GPScoord;
  for (byte j = 0; j < 4; j++)
  ...
  for (byte i = 0; i < 4; i++)
  {
    ...
    if (!apogee && velocity > maxVelocity)
    {
  ...
  if (settings.debugSerial)
  {
    Serial.println("Inflight Data Written");
  }
  // set the next channel
  if (activeRadio->FHSS)
  {
...
void postflightPacket(byte rxPacket[])
{
  ...
  // write to the SD card
  if (SDinit && parseSustainer && sustainerPostFlightWrite)
  {
    writePostflightData();
  }
  if (SDinit && parseBooster && boosterPostFlightWrite)
  {
    writePostflightData();
  }
