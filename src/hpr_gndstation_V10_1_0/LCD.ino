...
void startupLCD()
{
  ...
  lcd.print('\r');
  // Radio 1
  if (radio1.enable && radio1.status)
  {
    lcd.print(F("Radio 1: "));
    lcd.print(radio1.frq, 3);
    lcd.print(" MHz");
  }
  else if (radio1.enable && !radio1.status)
  {
    lcd.print(F("Radio 1: Failed!"));
  }
  else if (!radio1.enable)
  {
    lcd.print(F("Radio 1: Sleep"));
  }
  lcd.print('\r');
  // Radio 2
  if (radio2.enable && radio2.status)
  {
    lcd.print(F("Radio 2: "));
    lcd.print(radio2.frq, 3);
    lcd.print(" MHz");
  }
  else if (radio2.enable && !radio2.status)
  {
    lcd.print(F("Radio 2: Failed!"));
  }
  else if (!radio2.enable)
  {
    lcd.print(F("Radio 2: Sleep"));
  }
  delay(8000);
  ...
  //----------------------------------------
  lcd.print('\r');
  if (isnan(lastGPSlat) || lastGPSlat == 0.0)
  {
    lastGPSlat = 1234.5678;
  }
  ...
  //----------------------------------------
  lcd.print('\r');
  if (isnan(lastGPSlon) || lastGPSlon == 0.0)
  {
    lastGPSlon = 1234.5678;
  }
...
void preflightLCD()
{
  ...
  myString.trim();
  for (byte i = myString.length() + 1; i < 21; i++)
  {
    myString += " ";
  }
  // line 1
  myString += pyroTable[contCode];
  for (byte i = myString.length() + 1; i < 41; i++)
  {
    myString += " ";
  }
  // line 2
  ...
  for (byte i = myString.length() + 1; i < 61; i++)
  {
    myString += " ";
  }
  // line 3
  ...
  myString += " ft";
  for (byte i = myString.length() + 1; i < 81; i++)
  {
    myString += " ";
  }
  lcd.print(myString);
...
void inflightLCD()
{
  ...
  myString += " dBm";
  for (byte i = myString.length() + 1; i < 78; i++)
  {
    myString += " ";
  }
...
void parseCoord(float coord)
{
  ...
  strPosn = 0;
  while (dataString[strPosn] != '.')
  {
    strPosn++;
  }
  byte endPosn = strPosn + 1;
  while (dataString[endPosn] != '\0')
  {
    endPosn++;
  }
  while (endPosn >= strPosn - 2)
  {
    dataString[endPosn + 1] = dataString[endPosn];
    endPosn--;
  }
